...
$matches = [regex]::Match($assemblyName, 'RevitAddin\.Xunit\.(?<assemblyVersion>\d+\.\d+\.\d+)')
...
if ($commonExe) {
    ...
    $manifestToolCommand = "& '$destExePath' generate-xunit-manifest --output '$addinDir' --revit-version '$RevitVersion' --assembly '$installedAssemblyPath' --package-version '$assemblyVersion'"
